...
import { Users, Plus, Edit2, Trash2, Mail, Phone, Briefcase, Tag, ArrowLeft, X, Save } from "lucide-react";
...
export default function Contacts() {
  ...
  const loadContacts = () => {
    ...
    api.get("/contacts")
      ...
      .catch(err => {
        console.log(err);
        setMessage("Fehler beim Laden der Kontakte");
      });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(err.response?.data?.error || "Fehler beim Speichern des Kontakts");
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setMessage("Kontakt erfolgreich gelöscht!");
      loadContacts();
      setShowDeleteModal(false);
      setContactToDelete(null);
    } catch (err) {
      setMessage(err.response?.data?.error || "Fehler beim Löschen des Kontakts");
      setShowDeleteModal(false);
      setContactToDelete(null);
    }
  };
  ...
  return (
    <div className={`min-h-screen overflow-y-auto ${isDarkMode ? 'bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900' : 'bg-gradient-to-br from-gray-50 via-blue-50 to-gray-50'}`}>
      {/* Header */}
      <Navbar title="Meine Kontakte" subtitle={`${contacts.length} Kontakte gespeichert`} />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-20">
        ...
        <div className="flex items-center justify-start gap-4 mb-6">
          ...
          >
            ...
            <span className="font-semibold">Kontakt hinzufügen</span>
          </button>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className={`rounded-2xl p-8 max-w-md w-full shadow-2xl border animate-fade-in ${
            ...
            <p className={`mb-8 ${isDarkMode ? 'text-blue-200' : 'text-gray-600'}`}>
              Möchtest du diesen Kontakt wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.
            </p>

    ...
    <div className={`min-h-screen flex flex-col ${isDarkMode ? 'bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900' : 'bg-gradient-to-br from-gray-50 via-blue-50 to-gray-50'}`}>
      ...
      <div className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        ...
        <div className="flex items-center justify-start gap-4 mb-6">
          ...
          <button onClick={openAddModal} className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-700 hover:from-blue-600 hover:to-blue-800 text-white rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg">
        ...
        {events.length === 0 ? (
          <div className="text-center py-20 mt-16">
            <Calendar className={`w-20 h-20 mx-auto mb-4 opacity-50 ${isDarkMode ? 'text-blue-300' : 'text-gray-400'}`} />
            <h3 className={`text-2xl font-semibold mb-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{t("noEvents")}</h3>
            <p className={`mb-6 ${isDarkMode ? 'text-blue-200' : 'text-gray-600'}`}>{t("addFirstEvent")}</p>
            <button onClick={openAddModal} className="px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-700 hover:from-blue-600 hover:to-blue-800 text-white rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg font-semibold">
              {t("addEvent")}
            </button>
          </div>
        ) : (
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className={`rounded-2xl p-8 max-w-md w-full shadow-2xl border animate-fade-in ${isDarkMode ? 'bg-gradient-to-br from-blue-700 via-blue-800 to-blue-900 border-white/20' : 'bg-blue-500 border-gray-200'}`}>
            <div className="flex items-center justify-between mb-6">
              <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-white'}`}>
                {editingEvent ? t("editEvent") : t("newEvent")}
              </h2>
              <button onClick={closeModal} className={`p-2 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-blue-600' : 'hover:bg-blue-600'}`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className={`block text-sm font-medium mb-2 text-white`}>{t("title")} *</label>
                <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} required className="w-full px-4 py-3 rounded-lg border border-blue-300 text-white bg-blue-600 placeholder-blue-200 focus:ring-green-500 focus:outline-none focus:ring-2 focus:border-transparent" placeholder="z.B. Meeting mit Professor" />
              </div>

              <div>
                <label className={`block text-sm font-medium mb-2 text-white`}>{t("startTime")} *</label>
                <input type="datetime-local" value={formData.start} onChange={(e) => setFormData({ ...formData, start: e.target.value })} required className="w-full px-4 py-3 rounded-lg border border-blue-300 text-white bg-blue-600 focus:ring-green-500 focus:outline-none focus:ring-2 focus:border-transparent" />
              </div>

              <div>
                <label className={`block text-sm font-medium mb-2 text-white`}>{t("endTime")} *</label>
                <input type="datetime-local" value={formData.end} onChange={(e) => setFormData({ ...formData, end: e.target.value })} required className="w-full px-4 py-3 rounded-lg border border-blue-300 text-white bg-blue-600 focus:ring-green-500 focus:outline-none focus:ring-2 focus:border-transparent" />
              </div>

              <div>
                <label className={`block text-sm font-medium mb-2 text-white`}>{t("description")}</label>
                <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows="3" className="w-full px-4 py-3 rounded-lg border border-blue-300 text-white bg-blue-600 placeholder-blue-200 focus:ring-green-500 focus:outline-none focus:ring-2 focus:border-transparent" placeholder="Optionale Details zum Termin..." />
              </div>

              <div className="flex gap-3 pt-4">
                <button type="button" onClick={closeModal} className="flex-1 px-4 py-3 rounded-lg bg-blue-700 hover:bg-blue-800 text-white transition-all duration-200 font-medium">{t("cancel")}</button>
                <button type="submit" className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-green-500 hover:bg-green-600 text-white rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg font-semibold">
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className={`bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-2xl max-w-sm w-full`}>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={confirmDelete} className="flex-1 px-4 py-2 rounded-lg bg-green-500 hover:bg-green-600 text-white transition-all duration-200 font-semibold">{t("delete")}</button>
            </div>
          </div>
        </div>
      )}
